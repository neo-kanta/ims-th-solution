...
COMMENT ON TABLE investment__asset_subtypes IS 'Sub-classification under an asset class (COMMON_STOCK, EQUITY_ETF, MUTUAL_FUND, GOV_BOND, etc.).';
...
COMMENT ON TABLE investment__regions IS 'Macro investment regions (APAC, EMEA, AMER, GLOBAL, THAILAND, etc.).';
...
COMMENT ON TABLE investment__countries IS 'Investment-context country list. Independent of any system-level locale data.';
...
COMMENT ON TABLE  investment__sectors IS 'GICS-compatible sector hierarchy (Sector -> Industry Group -> Industry -> Sub-Industry).';
...
COMMENT ON TABLE investment__fund_categories IS 'Fund / portfolio category (EQUITY_FUND, BOND_FUND, MIXED_FUND, MMF_FUND, INDEX_FUND, ...).';
...
COMMENT ON TABLE investment__investment_styles IS 'Investment style classification (GROWTH, VALUE, BLEND, INCOME, INDEX, ACTIVE).';
