...
watch(
  () => t("portfolio.workspaceTabs.holdings"),
  (newTitle) => {
...
async function loadInstrumentLabels(items: ApiHoldingV2[]) {
  const ids = [...new Set(items.map((holding) => holding.instrument_id).filter((id): id is string => Boolean(id)))];
...
async function loadHoldings() {
  ...
  try {
    ...
    await loadInstrumentLabels(holdings.value);
  } catch (err) {
    holdings.value = [];
    error.value = err instanceof Error ? err.message : t("portfolio.holdings.errorTitle");
  } finally {
    loading.value = false;
  }
}
      ...
      <table v-else class="portfolio-holdings__table">
        ...
        <tbody>
          <tr v-for="h in holdings" :key="h.instrument_id">
            <td>{{ labelForInstrument(h.instrument_id) }}</td>
            <td>{{ h.quantity }}</td>
