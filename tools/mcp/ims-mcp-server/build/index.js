...
const PROJECT_ROOT = process.env.IMS_PROJECT_ROOT ?? path.resolve(process.cwd());
...
const DOMAIN_RULES = [
    // Workflow rules
    {
        module: "workflow",
        code: "WF-001",
        description: "Investment transactions must not occur before Investment Day Start",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "workflow",
        code: "WF-002",
        description: "Manager Approval can only happen after all required transactions/reviews for that day are done",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "workflow",
        code: "WF-003",
        description: "After Manager Approval, managers can no longer perform transactions for that day",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "workflow",
        code: "WF-004",
        description: "Transaction Closing and Accounting Closing are separate states — cannot skip",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "workflow",
        code: "WF-005",
        description: "Rollback/cancel operations must be modeled explicitly with audit trail",
        severity: "error",
        enforcement: "both",
    },
    {
        module: "workflow",
        code: "WF-006",
        description: "All workflow actions must log actor, time, contract, action, and result",
        severity: "error",
        enforcement: "both",
    },
    // Stock Investment rules
    {
        ...
        code: "SI-001",
        description: "Minimum trading unit validation — order quantity must be a multiple of the minimum trading unit",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "stock_investment",
        code: "SI-002",
        description: "Price tick validation — order price must align with exchange tick size rules",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "stock_investment",
        code: "SI-003",
        description: "Buy/sell recommendation must be linked to an approved analysis report",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "stock_investment",
        code: "SI-004",
        description: "Sell quantity must not exceed allowed inventory (available shares)",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "stock_investment",
        code: "SI-005",
        description: "Submission must go through approval workflow before execution",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "stock_investment",
        code: "SI-006",
        description: "If report approval is required, investment decision cannot bypass it",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "stock_investment",
        code: "SI-007",
        description: "Contract/manager visibility restrictions — users can only see contracts they are authorized for",
        severity: "error",
        enforcement: "both",
    },
    {
        module: "stock_investment",
        code: "SI-008",
        description: "Stock pool / investment scope check — instrument must be in the fund's allowed investment universe",
        severity: "error",
        enforcement: "server",
    },
    // Leave / Delegation rules
    {
        ...
        code: "LD-001",
        description: "Users on approved leave are restricted from normal login/operations",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "leave_delegation",
        code: "LD-002",
        description: "Delegated users inherit functional AND data permissions during the leave window only",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "leave_delegation",
        code: "LD-003",
        description: "Agent setup supports priority-based delegation — up to 6 agents per contract",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "leave_delegation",
        code: "LD-004",
        description: "Leave, temporary leave, and leave cancellation flows must all exist",
        severity: "error",
        enforcement: "server",
    },
    // Permissions rules
    ...
    },
    {
        module: "permissions",
        code: "PM-002",
        description: "Data permissions are per-contract/fund — only authorized users can see or act on contract data",
        severity: "error",
        enforcement: "both",
    },
    {
        module: "permissions",
        code: "PM-003",
        description: "Function permissions are assigned to groups, users are assigned to groups",
        severity: "error",
        enforcement: "both",
    },
    // Compliance / IRG hooks
    {
        module: "compliance",
        code: "IRG-001",
        description: "Pre-trade checking must be enforced before any order execution",
        severity: "error",
        enforcement: "server",
    },
    {
        module: "compliance",
        code: "IRG-002",
        description: "Extension points must exist for blacklist/whitelist, investment ratio, instrument restriction, credit/rating rules",
        severity: "warning",
        enforcement: "server",
    },
];
...
// TOOL: db_query
// ===========================
server.tool("db_query", "Run a read-only SQL query against the IMS PostgreSQL database. Returns rows as JSON. Only SELECT statements are allowed.", {
...
// TOOL: db_schema
// ===========================
server.tool("db_schema", "List all tables in the IMS database with their columns, data types, and constraints. Useful for understanding the current database structure.", {
    schema_name: z
        .string()
        .optional()
        .default("public")
...
// TOOL: db_table_info
// ===========================
server.tool("db_table_info", "Get detailed information about a specific table: columns, indexes, foreign keys, row count estimate.", {
    ...
    try {
        ...
        `, [table_name]);
        // Row count estimate
        const countEst = await safeQuery(`SELECT reltuples::bigint AS estimate FROM pg_class WHERE relname = $1`, [table_name]);
        // Comments
...
// TOOL: db_migrations
// ===========================
server.tool("db_migrations", "Show the migration history from the schema_migrations table — which migrations have been applied to the database.", {}, async () => {
    try {
        const result = await safeQuery(`SELECT version, dirty FROM schema_migrations ORDER BY version DESC LIMIT 50`);
        return {
...
// ===========================
server.tool("workflow_states", "Show the IMS workflow state machine: all valid states, forward transitions, rollback transitions, and business rules.", {}, async () => {
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify({
                    ...
                    lifecycle: "NOT_STARTED → INVESTMENT_DAY_START → MANAGER_APPROVAL → TRANSACTION_CLOSING → ACCOUNTING_CLOSING → (next day)",
...
// ===========================
server.tool("workflow_validate", "Check if a workflow state transition is valid. Returns whether it's a forward move, rollback, or invalid.", {
    from_state: z
    ...
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify({ from_state, to_state, status, detail }, null, 2),
            },
        ],
    };
});
...
// ===========================
server.tool("perm_user_rights", "Show all permissions for a user: their groups, function rights, and data rights (contract visibility).", {
    username: z.string().describe("Username to look up"),
}, async ({ username }) => {
    try {
        const userResult = await safeQuery(`SELECT id, username, display_name, is_active, is_on_leave FROM iam_users WHERE username = $1`, [username]);
        ...
        `, [userId]);
        // Data rights
        const dataRights = await safeQuery(`SELECT contract_id, is_granted FROM permissions_data_rights WHERE user_id = $1`, [userId]);
        return {
            content: [
                {
                    type: "text",
...
// TOOL: perm_check
// ===========================
server.tool("perm_check", "Check if a specific user has a specific function permission code. Returns granted/denied with context.", {
    username: z.string().describe("Username to check"),
    permission_code: z
        .string()
        .describe("Permission code (e.g. 'WORKFLOW_START_DAY', 'INVESTMENT_CREATE_ORDER')"),
...
// TOOL: perm_groups
// ===========================
server.tool("perm_groups", "List all permission groups with their members and assigned function rights.", {}, async () => {
    try {
        const groups = await safeQuery(`SELECT id, name, description, is_active FROM permissions_groups ORDER BY name`);
        const details = [];
        for (const group of groups.rows) {
          ...
          WHERE ag.group_id = $1
          `, [group.id]);
            const rights = await safeQuery(`SELECT permission_code, is_granted FROM permissions_function_rights WHERE group_id = $1 ORDER BY permission_code`, [group.id]);
            details.push({
                ...group,
...
// TOOL: domain_rules
// ===========================
server.tool("domain_rules", "List the business validation rules for a specific module (or all modules). These are the IMS domain rules that the backend MUST enforce.", {
    module: z
        .string()
        .optional()
        .describe("Module name to filter (workflow, stock_investment, leave_delegation, permissions, compliance). Leave empty for all."),
}, async ({ module }) => {
...
// TOOL: audit_log
// ===========================
server.tool("audit_log", "Query the audit log table for recent entries. Useful for debugging workflow actions, permission changes, and domain events.", {
...
// ===========================
server.tool("project_modules", "List all backend domain modules, their directory structure, and whether they have route registrations, domain entities, or TODO markers.", {}, async () => {
    try {
...
// ===========================
server.tool("project_migrations", "List all migration files in the database/migrations directory (filesystem view, not DB state).", {}, async () => {
    try {
        ...
        for (const m of migrations) {
            ...
            if (m.direction === "up")
                grouped[m.version].up = m.filename;
            if (m.direction === "down")
                grouped[m.version].down = m.filename;
        }
        return {
            content: [
                {
...
// ===========================
server.tool("project_api_routes", "Scan the backend codebase for registered API routes by looking for chi router patterns in transport/router.go and module.go files.", {}, async () => {
    try {
        const internalDir = path.join(BACKEND_ROOT, "internal");
        const entries = await fs.readdir(internalDir, { withFileTypes: true });
        const routes = [];
        for (const entry of entries) {
            ...
            for (const filePath of filesToCheck) {
                try {
                    ...
                    const routePatterns = content.match(/r\.(Get|Post|Put|Delete|Patch|Route|Group|Handle|Method)\s*\(\s*"[^"]*"/g);
...
server.resource("airead", "file:///AIREAD.md", async (uri) => {
    try {
        const content = await fs.readFile(path.join(PROJECT_ROOT, "AIREAD.md"), "utf-8");
        return {
