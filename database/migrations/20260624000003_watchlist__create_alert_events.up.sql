...
CREATE INDEX idx_watchlist_alerts_personal
    ON watchlist_alert_events (owner_user_id, created_at DESC)
    WHERE scope_type = 'PERSONAL';

CREATE INDEX idx_watchlist_alerts_portfolio
    ON watchlist_alert_events (portfolio_id, created_at DESC)
    WHERE scope_type = 'PORTFOLIO';

CREATE INDEX idx_watchlist_alerts_rule
    ON watchlist_alert_events (threshold_rule_id, created_at DESC);

CREATE INDEX idx_watchlist_alerts_acknowledged
    ON watchlist_alert_events (acknowledged_at, created_at DESC);

CREATE INDEX idx_watchlist_alerts_security
    ON watchlist_alert_events (security_id, created_at DESC);
